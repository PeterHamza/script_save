...
echo "Save database"
ssh $user@$nextcloud_host 'mysqldump --single-transaction -u $db_user -p$db_password $db_name > /data/backup/$database_backup'
sudo /usr/bin/rsync --rsync-path="sudo /usr/bin/rsync" -av $user@$nextcloud_hosthome:/data/backup/database/$database_backup /data/backup/$database_backup
